...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    private StateInfo DoAddClip(StateInfoScriptableObject scriptable)
    {
        //Start new State
        StateInfo newState = m_States.InsertState();
        newState.Initialize(scriptable);
        //Find at which input the state will be connected
        int index = newState.index;
    ...
    public bool AddClip(StateInfoScriptableObject scriptable)
    {
        DoAddClip(scriptable);
        UpdateDoneStatus();
        InvalidateStates();
    ...
    private void UpdateStates(float deltaTime)
    {
        ...
        for (int i = 0; i < m_States.Count; i++)
        {
            ...
            //if(state.enabled && state.OnEnd != null)
            //{
            //    state.OnEnd(null,null);
            //}
